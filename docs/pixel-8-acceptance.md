...
The final storage/suggestion/recovery residual gate **PASSED**. Publication verification then cleared both previously outstanding accessibility cases: each passed isolated, the complete accessibility class passed 8/8, and both passed inside the one complete 85-case device run. That full run finished **83/85** with two different failures. The capture teardown timeout passed in the single combined focused rerun; the real-keyguard case repeated at its emulator setup wait, received an instrumentation-only timing correction, and then passed focused 1/1. A second full-device run was not scheduled after that test-only change, so the latest full-suite result remains formally failed rather than being presented as clean. The earlier complete 81-case device suite passed at `643cb4e`. The APK is a debug build, not a signed production release; the complete physical MVP acceptance remains pending.
...
The clean checkpoint at `b7def71` completed compilation, **190/190** JVM tests, and APK assembly. Its device phase was stopped as obsolete at **55/85** after these two failures: `AccessibilityAndBidiTest.reusedWindowContainsOnlyUnlockSemanticsAtTheActualDismissalRequest` (unlock node not displayed) and `AccessibilityAndBidiTest.recoverableOnboardingErrorIsMonochromeAndAnnouncedPolitely` (`PixelCopy` timeout). This remains an incomplete/failed historical attempt; no environmental cause is claimed. Final `gradlew.bat testDebugUnitTest lintDebug assembleDebug` at production revision `20b20c8` passed in 42s with **190 unit tests**, **0 failures, 0 errors, 0 skipped**, and successful APK assembly.

For publication verification at `a1c3cb1`, the exact reused-window case passed **1/1** in 29s and the exact onboarding-error case passed **1/1** in 28s. The complete `AccessibilityAndBidiTest` class then passed **8/8** in 2m 08s. The single complete device run finished all **85/85 attempted**, **83 passed, 2 failed, 0 skipped**, in 17m 17s; both requested accessibility cases passed. Its two different failures were `CaptureActivityTest.finishingStopFlushSurvivesViewModelClearing` and `ReviewUnlockRoutingTest.realAdapterReportsTheActualCredentialFreeKeyguardAsLocked`, each at a five-second setup/lifecycle wait. In the one combined focused rerun, the capture case passed and only the real-keyguard setup wait failed again. A direct API-37 emulator probe observed the credential-free keyguard transition become visible around four seconds after enable-plus-sleep. The harness now allows ten seconds for that asynchronous precondition, without changing either the required locked state or the subsequent real-adapter assertion; the corrected exact case passed **1/1** in 23s. No production source or APK changed, and no second full-device suite was run after this instrumentation-only correction.
...
Generated evidence: `app/build/test-results/testDebugUnitTest/TEST-*.xml` currently totals 190 passing tests; `app/build/outputs/androidTest-results/connected/debug/TEST-Pixel_8_API_37(AVD) - 17-_app-.xml` currently contains the final focused 1-case keyguard pass; and `app/build/reports/lint-results-debug.xml` contains the 0-error/16-warning result. These artifacts are intentionally ignored in Git. The command chronology above distinguishes historical full-suite results, the incomplete 55/85 checkpoint, the completed 83/85 run, and the current focused XML rather than presenting focused evidence as a full-device pass.
